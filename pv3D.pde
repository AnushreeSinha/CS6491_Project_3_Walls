...
class vec { 
  ...
  float x=0,y=0,z=0; //VECTOR COORDINATES IN 3D
  
   vec () {}; //CONTRRUCTOR 1
   vec (float px, float py, float pz) {x = px; y = py; z = pz;};// CONSTRUCTOR 2
    } // end class vec
...
//LINEAR INTERPOLATED POINT BETWEEN A AND B
...
//weighted aVERAGE OF TWO POINTS - a+b = 1
...
//PONT IN THE 2D COORDINATE SYSTEM, O IS THE ORIGIN AND I AND J ARE THE I AND J UNIT VETCORS ALONG X AND Y AXIS RESPECTIVELY
...
//SET OF POINTS - USE FOR DART MESH
...
//INTERPRET AS SQARE ROOT OF THE MEAN SQAURED DISTANCE BETWEEN U AND V
float det3(vec U, vec V) {return sqrt(d(U,U)*d(V,V) - sq(d(U,V))); };                                       // U|V det product

//SCALAR TRIPLE PRODUCT - USE FOR VOLUME OF PARALLELOPIPED AND TO TEST IF THREE POINTS ARE COPLANAR
...
//USE THESE FOR YOUR CLOSEST SWING STUFF
...
//CHECKING FOR CLOCKWISE - USE THIS
boolean cw(vec U, vec V, vec W) {return m(U,V,W)>0; };                                               // (UxV)*W>0  U,V,W are clockwise

//IMPORTANT INTERPRETATION - READ THE COMMENT PROVIDED
...
float disToLine(pt P , pt A , vec B){return det3(U(B),V(A,P)); };

pt projectionOnLine(pt P, pt A, pt B) {return P(A,dot(V(A,B),V(A,P))/dot(V(A,B),V(A,B)),V(A,B));}

// ===== rotate 
//CLOCKWISE ROTATION IN THE X-Y PLANE.
vec R(vec V) {return V(-V.y,V.x,V.z);} // rotated 90 degrees in XY plane

//ROTATION OF A POINT P ABOUT A POINT G BY AN ANGLE a IN THE PLANE PARALLEL TO <IJ> - READ AGAIN
...
//POINT Q ROTATED IN THE X-Y PLANE ABOUT THE ORIGIN (0,0,0) BY ANGLE  a
...
//POINT Q ROTATED ABOUT C IN THE X-Y PLANE BY ANGLE a
...
//THE FOLLOWING ARE VERY IMPORTANT CLASS OF FUNCTIONS AND I DON'T IMPLEMENT THEM SO WELL - READ AGAIN - READ COMMENTS 
// ===== render
...
//EDGE BETWEEN TWO POINTS P AND Q
...
//RENDERS THE COORDINATE SYSTEM - PRACTISE THIS
